...
- `:core-database` — Room, entités, DAOs, migrations explicites versionnées dès le premier schéma. **Première version implémentée (2026-08-29)** : entité/DAO `DiscoveredCellEntity`/`DiscoveredCellDao` (clé primaire `(h3Index, trustStatus)`, Normal et Certified jamais dans la même ligne), `WorldDiscoveryDatabase` v1, `RoomDiscoveredCellRepository`. Pas encore de tracking, de calcul de pourcentage ni de synchronisation backend.
- `:core-location` — permissions, capture GPS en arrière-plan (service/WorkManager), échantillonnage adaptatif. *(Non implémenté — hors périmètre de la fondation locale du moteur.)*
- `:core-discovery-engine` — logique pure Kotlin (sans dépendance Android), testable isolément : classification déplacement/présence, conversion H3, agrégation multi-niveaux de précision. Voir [discovery-engine.md](discovery-engine.md). **Première version implémentée (2026-08-29)** : `DiscoveryEngineVersion` (constante centrale unique pour `engineVersion`/résolution H3 canonique — jamais de littéral `12` dispersé ailleurs), conversion coordonnée → `CanonicalCell` via `H3JavaCellConverter` (bibliothèque `com.uber:h3`, jamais réimplémentée), modèle `DiscoveryEvent`/`DiscoveredCell`/`Provenance`/`TrustStatus`, règle de fusion déterministe `DiscoveredCellMerger`, cas d'usage `SubmitDiscoveryObservation`. Classification déplacement/présence et agrégation Easy/Standard/Hard restent non implémentées (incréments suivants).
  - Note technique : `com.uber:h3` (bibliothèque générique, utilisée ici pour la portabilité JVM/tests) ne embarque pas les binaires natifs Android. Une exécution réelle sur appareil Android devra utiliser `com.uber:h3-android` (ou une extraction manuelle des binaires natifs `android-*`) — non vérifié dans cette phase, à valider avant l'implémentation du tracking réel.
