...
**Status:** DECIDED / NOT IMPLEMENTED for worldwide coverage; France's rendering-only OSM polygon is IMPLEMENTED and PHYSICALLY VALIDATED as a prototype (see `PROJECT_STATUS.md` §17), not yet generalized; France's Region (`ADMIN_1`)/Department (`ADMIN_2`) classification geometry is IMPLEMENTED (13 metropolitan regions; Nouvelle-Aquitaine's 12 departments only — see `PROJECT_STATUS.md`'s Phase H), sourced directly from OpenStreetMap (not Overture Divisions — see `tools/geo/README.md`'s "Future sources" section for why), NOT yet physically validated on-device; ingestion and mapping pipeline for other countries/levels/regions' departments remains ENGINEERING DESIGN REQUIRED / a mechanical data-population follow-up respectively.
