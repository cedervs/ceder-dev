...
**Status:** DECIDED / NOT IMPLEMENTED for worldwide coverage; France's rendering-only OSM polygon is IMPLEMENTED and PHYSICALLY VALIDATED as a prototype (see `PROJECT_STATUS.md` §17), not yet generalized; France's Region (`ADMIN_1`)/Department (`ADMIN_2`) classification geometry is IMPLEMENTED with full metropolitan coverage (13 metropolitan regions; all 96 of their departments, including Corse's two — see `PROJECT_STATUS.md`'s Phase H and Phase FH-1), sourced directly from OpenStreetMap (not Overture Divisions — see `tools/geo/README.md`'s "Future sources" section for why); the complete 13/96 dataset is covered by automated/data validation (generator provenance, hierarchy/geometry/interior-point tests — see `PROJECT_STATUS.md`'s Phase FH-1), and functional hierarchy behavior (Region/Department navigation, both a visited and an unvisited branch) has PASSED physical validation on-device — see `PROJECT_STATUS.md`'s Phase FH-1 "second physical validation" entry — but this was a sampled functional check (Nouvelle-Aquitaine/Haute-Vienne's existing visited branch, plus Grand Est and one of its departments as an unvisited branch), not individual physical testing of every one of the 13 regions/96 departments; final visual calibration (labels, contrast, zoom thresholds, basemap) remains future work; ingestion and mapping pipeline for other countries remains ENGINEERING DESIGN REQUIRED.
